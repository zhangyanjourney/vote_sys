...
<form class="form-horizontal form we7-form" action="" method="post">
	...
	<?php  if(is_array($menus)) { foreach($menus as $name => $sections) { ?>
		<?php  if($sections['section'] && $name == 'account' && in_array($account['type'], array(ACCOUNT_TYPE_OFFCIAL_NORMAL, ACCOUNT_TYPE_OFFCIAL_AUTH))) { ?>
			<div class="we7-page-title"><?php  echo $sections['title'];?><span class="color-default font-sm">（<i class="fa fa-exclamation-circle"></i> 默认未勾选任何菜单时，用户拥有全部权限）</span>
			</div>
			<?php  if(is_array($sections['section'])) { foreach($sections['section'] as $section_id => $section) { ?>
				<?php  if(!in_array($section_id, array('platform_module'))) { ?>
					...
					<?php  if(is_array($section['menu'])) { foreach($section['menu'] as $menu_id => $menu) { ?>
						<?php  if(is_array($menu['permission_display']) && !in_array($account['type'], $menu['permission_display'])) { ?><?php  continue;?><?php  } ?>
						...
						<td class="text-left vertical-middle" style="overflow:inherit;">
							<div class="dropdown system-select-dropdown">
								<span>
									...
									<label for="check-child-<?php  echo $menu_id;?>" class="we7-margin-horizontal-none" data-toggle="tooltip"><?php  echo $menu['title'];?><?php  if(!empty($menu['sub_permission'])) { ?><span class="caret"></span><?php  } ?></label>
								</span>
					...
					<tr class="text-left">
						<th colspan="4" class="text-left">
							<div class="">
								...
								<label for="check-<?php  echo $section_id;?>" class=" we7-margin-horizontal-none"><?php  echo $section['title'];?><span class="caret"></span></label>
							</div>
						</th>
					</tr>
					</thead>
						...
						<td class="text-left vertical-middle" style="overflow:inherit;">
							<div class="dropdown system-select-dropdown">
								...
								<ul class="dropdown-menu">
									<?php  if(is_array($menu['sub_permission'])) { foreach($menu['sub_permission'] as $sub_permission) { ?>
									<li class="text-left">
										<input id="check-child-<?php  echo $sub_permission['permission_name'];?>"  we7-check-all="1" type="checkbox" value="<?php  echo $sub_permission['permission_name'];?>" <?php  if(in_array($sub_permission['permission_name'], $user_menu_permission_wxapp)) { ?>checked<?php  } ?> name="wxapp[]">
										<label class="we7-margin-horizontal-none" for="check-child-<?php  echo $sub_permission['permission_name'];?>">
											<?php  echo $sub_permission['title'];?>
										</label>
									</li>
	...
	<table class="table we7-table table-hover vertical-middle we7-form">
		<div class="we7-page-title">应用<span class="color-default font-sm">（<i class="fa fa-exclamation-circle"></i> 默认未勾选任何菜单时，用户拥有全部权限）</span></div>
		<thead>
			<tr>
				<th colspan="4" class="text-left">
					<div class="">
					...
					<td  class="text-left vertical-middle" style="overflow:inherit;width:25%">
						<div class="dropdown system-select-dropdown">
							<span class="" data-id="<?php  echo $k;?>">
								...
								<label for="check-module-<?php  echo $k;?>" ><?php  echo mb_substr($m['title'], 0, 12, 'utf-8')?><span class="caret"></span></label>
							</span>
